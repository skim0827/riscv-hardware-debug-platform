...
(
    ...
    // Fetch handshake shared by all replicas.
...
logic in_fetch_r_0,   in_fetch_r_1,   in_fetch_r_2;
...
main_fsm u_fsm_0 (
    ...
    .RegWrite(RegWrite_0), .MemWrite(MemWrite_0), .MemRead(MemRead_0),
    .ResultSrc(ResultSrc_0), .ALUSrcB(ALUSrcB_0), .ALUSrcA(ALUSrcA_0),
    .ForceAdd(ForceAdd_0),
    .stall(stall),
    .fetch_done(fetch_done),
    .in_fetch_r(in_fetch_r_0)
);
...
main_fsm u_fsm_1 (
    ...
    .RegWrite(RegWrite_1), .MemWrite(MemWrite_1), .MemRead(MemRead_1),
    .ResultSrc(ResultSrc_1), .ALUSrcB(ALUSrcB_1), .ALUSrcA(ALUSrcA_1),
    .ForceAdd(ForceAdd_1),
    .stall(stall),
    .fetch_done(fetch_done),
    .in_fetch_r(in_fetch_r_1)
);
...
main_fsm u_fsm_2 (
    ...
    .RegWrite(RegWrite_2), .MemWrite(MemWrite_2), .MemRead(MemRead_2),
    .ResultSrc(ResultSrc_2), .ALUSrcB(ALUSrcB_2), .ALUSrcA(ALUSrcA_2),
    .ForceAdd(ForceAdd_2),
    .stall(stall),
    .fetch_done(fetch_done),
    .in_fetch_r(in_fetch_r_2)
);

// TMR majority vote on all outputs
...
assign in_fetch_r = (in_fetch_r_0 & in_fetch_r_1) | (in_fetch_r_0 & in_fetch_r_2) | (in_fetch_r_1 & in_fetch_r_2);
...
// TMR error flags any replica disagreement.
assign tmr_error =
    ...
    (in_fetch_r_0 ^ in_fetch_r_1) | (in_fetch_r_1 ^ in_fetch_r_2) |
