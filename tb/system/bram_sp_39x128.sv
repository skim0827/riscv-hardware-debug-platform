`timescale 1ns/1ps
// bram_sp_39x128.sv — Simulation model for Vivado Block Memory Generator IP

module bram_sp_39x128 #(
    parameter MEM_INIT = ""  
)(
    input  logic        clka,
    input  logic        wea,
...
// Temporary array for $readmemh (32-bit words)
...
// Single-port synchronous BRAM — write-first, 1-cycle read latency
always_ff @(posedge clka) begin
    if (wea) begin
        mem[addra] <= dina;
        douta      <= dina;   // write-first: output new data same cycle
    end else begin
        douta <= mem[addra];
    end
end
