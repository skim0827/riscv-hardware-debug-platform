`timescale 1ns/1ps
// bram_sp_39x128_d.sv — Simulation model for DMEM Vivado BRAM IP
// Identical behaviour to bram_sp_39x128 but a distinct module name so
// Vivado can use a separate IP instance (bram_sp_39x128_d) with its own
// COE init file (uart_hello_dmem.coe).

module bram_sp_39x128_dmem #(
    parameter MEM_INIT = ""
)(
    input  logic        clka,
    input  logic        wea,
...
logic [31:0] raw_words [0:127];

initial begin
    for (int i = 0; i < 128; i++) mem[i] = 39'b0;

    if (MEM_INIT != "") begin
        $readmemh(MEM_INIT, raw_words);
        for (int i = 0; i < 128; i++)
            mem[i] = {ecc_encode(raw_words[i]), raw_words[i]};
        $display("[bram_d_sim] Loaded %s", MEM_INIT);
    end
end
